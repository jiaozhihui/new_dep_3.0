 ...
 * 分镜头 + 标签合成
 */
object TestDemo extends Logging {
  ...
  def fun(sparkSession: SparkSession,vid: String): Unit = {
    ...
    sparkSession.sql("cache table ccc")

    /**
     * 单标签合成
     */

    val rst1 = sparkSession.sql("select * from ccc")
      .toJSON
      .rdd
    ...
    rst1.saveJsonToEs("test/doc", Map(
     ...
     * Cleaned2 多标签合成
     */
    val rst = sparkSession.sql("select * from ccc")
      .rdd
    ...
    //      logWarning("第" + i + "次合并任务完成，增加了" + rddNum + "段视频")
